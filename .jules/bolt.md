...
**Learning:** Sending files via `FormData` (multipart/form-data) is ~25% smaller than Base64-encoded JSON payloads and avoids blocking the main thread with `FileReader.readAsDataURL()`.
**Action:** Always prefer `FormData` for file uploads in Next.js App Router. Use `request.formData()` on the server to handle files efficiently.
