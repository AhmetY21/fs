...
// ⚡ Bolt Optimization: Cache static data mapping to prevent redundant computation across API requests
const CACHED_RULES_TEXT = FENG_SHUI_RULES.map(r =>
    `[${r.id}] (${r.category}, severity:${r.severity}/5) ${r.principle}`
).join('\n');

const CACHED_ELEMENTS_TEXT = Object.entries(FIVE_ELEMENTS).map(([name, el]) =>
    `${name.toUpperCase()}: colors=${el.colors.join(',')}, directions=${el.direction.join(',')}, enhances=${el.enhances}, weakens=${el.weakens}`
).join('\n');

const CACHED_SYSTEM_PROMPT = `You are an expert Feng Shui consultant with deep knowledge of both Form School and Compass School Feng Shui. You are analyzing a room photograph to provide a professional Feng Shui assessment.
