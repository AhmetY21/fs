...
// Cache the compiled rules and elements text to prevent redundant computation
// during high-traffic /api/analyze requests
const RULES_TEXT_CACHE = FENG_SHUI_RULES.map(r =>
    `[${r.id}] (${r.category}, severity:${r.severity}/5) ${r.principle}`
).join('\n');

const ELEMENTS_TEXT_CACHE = Object.entries(FIVE_ELEMENTS).map(([name, el]) =>
    `${name.toUpperCase()}: colors=${el.colors.join(',')}, directions=${el.direction.join(',')}, enhances=${el.enhances}, weakens=${el.weakens}`
).join('\n');

// Fully cache the resulting system prompt
