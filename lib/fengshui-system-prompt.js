...
// ⚡ Bolt Optimization: Cache the complex generated string at the module level
// This prevents redundant computation and object allocations (map/join) on every API request.
const _rulesText = FENG_SHUI_RULES.map(r =>
    `[${r.id}] (${r.category}, severity:${r.severity}/5) ${r.principle}`
).join('\n');

const _elementsText = Object.entries(FIVE_ELEMENTS).map(([name, el]) =>
