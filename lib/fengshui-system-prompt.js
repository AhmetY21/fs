...
const cachedRulesText = FENG_SHUI_RULES.map(r =>
    `[${r.id}] (${r.category}, severity:${r.severity}/5) ${r.principle}`
).join('\n');

const cachedElementsText = Object.entries(FIVE_ELEMENTS).map(([name, el]) =>
...
export function buildAnalysisSystemPrompt() {
    // ⚡ Bolt Optimization: Use module-level constants for static mappings
    // This avoids redundant string concatenation and iteration on every request.
    const rulesText = cachedRulesText;
    const elementsText = cachedElementsText;
