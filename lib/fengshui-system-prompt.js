...
// Pre-calculate the expensive, static system prompt string at module load time.
// This turns an O(N+M) string generation/array mapping operation on EVERY API call
// into an O(1) constant return, saving memory allocations and CPU cycles.
const PRECALCULATED_RULES_TEXT = FENG_SHUI_RULES.map(r =>
    `[${r.id}] (${r.category}, severity:${r.severity}/5) ${r.principle}`
).join('\n');

const PRECALCULATED_ELEMENTS_TEXT = Object.entries(FIVE_ELEMENTS).map(([name, el]) =>
    `${name.toUpperCase()}: colors=${el.colors.join(',')}, directions=${el.direction.join(',')}, enhances=${el.enhances}, weakens=${el.weakens}`
).join('\n');

const PRECALCULATED_ANALYSIS_SYSTEM_PROMPT = `You are an expert Feng Shui consultant with deep knowledge of both Form School and Compass School Feng Shui. You are analyzing a room photograph to provide a professional Feng Shui assessment.

## YOUR KNOWLEDGE BASE — ONLY reference these rules. Do NOT invent or hallucinate Feng Shui tips.

### Feng Shui Principles Library:
${PRECALCULATED_RULES_TEXT}

### Five Elements Theory:
${PRECALCULATED_ELEMENTS_TEXT}
...
export function buildAnalysisSystemPrompt() {
    return PRECALCULATED_ANALYSIS_SYSTEM_PROMPT;
}
