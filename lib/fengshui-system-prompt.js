 ...
 */
// ⚡ Bolt: Cache system prompt to prevent redundant computation
// Extracting complex string generation and data mapping into module-level
// constants effectively caches the result for the lifetime of the serverless
// function, preventing redundant computation on every request.
const RULES_TEXT = FENG_SHUI_RULES.map(r =>
    `[${r.id}] (${r.category}, severity:${r.severity}/5) ${r.principle}`
).join('\n');

const ELEMENTS_TEXT = Object.entries(FIVE_ELEMENTS).map(([name, el]) =>
    `${name.toUpperCase()}: colors=${el.colors.join(',')}, directions=${el.direction.join(',')}, enhances=${el.enhances}, weakens=${el.weakens}`
).join('\n');

const SYSTEM_PROMPT = `You are an expert Feng Shui consultant with deep knowledge of both Form School and Compass School Feng Shui. You are analyzing a room photograph to provide a professional Feng Shui assessment.
