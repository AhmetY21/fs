...
## 2025-03-10 - [Accessible Copy Feedback]
**Learning:** Copy buttons without state feedback cause uncertainty. Adding a temporary "Copied!" state with `aria-live="polite"` ensures both visual and screen reader users know the action succeeded.
**Action:** Always provide temporary accessible state feedback for clipboard actions (e.g., via a reusable `CopyButton` component that uses `aria-live="polite"`).
