...
**Learning:** Canvas-based spatial visualizations (like room overlays) are invisible to screen readers, hiding crucial location data.
**Action:** Generate semantic descriptions (e.g., "top-left", "center") from raw coordinates and expose them via `aria-label` or `aria-description` on the canvas element.
