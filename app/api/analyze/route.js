...
export async function POST(request) {
    try {
        ...
        // Securely extract IP: prioritize request.ip/x-real-ip, fallback to the LAST element of x-forwarded-for
        // to prevent spoofing bypasses where a user prepends a fake IP.
        const forwardedFor = request.headers.get('x-forwarded-for');
        const ip = request.ip ||
            request.headers.get('x-real-ip') ||
            (forwardedFor ? forwardedFor.split(',').pop().trim() : '127.0.0.1');
