    ...
    useEffect(() => {
        ...
        img.onload = () => {
            ...
            if (commandPosition && commandPosition.primary_furniture) {
                // ⚡ Bolt: Extract expensive string transformation outside of loop to prevent redundant computation
                const primaryFurnitureTarget = commandPosition.primary_furniture.toLowerCase();
                const mainElement = elements.find(
                    el => el.label?.toLowerCase().includes(primaryFurnitureTarget) ||
                        el.type?.toLowerCase().includes(primaryFurnitureTarget)
                );
                if (mainElement) {
